...
class LibriSpeechDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        """
            data_type \in ['train100', 'train360', 'train960', 'dev', 'test']
        """

        self.spect_func = torchaudio.transforms.Spectrogram(n_fft=n_fft)

        if data_type == "train100":
            self.list_url = [clean_path + "train-clean-100"]
        elif data_type == "train360":
            self.list_url = [clean_path + "train-clean-360"]
        elif data_type == "train500":
            self.list_url += [other_path + "train-other-500"]
        elif data_type == "dev":
